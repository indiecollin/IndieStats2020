...
class PlayerDetailsRivals extends Component{
    ...
    getRivalData(player, rival, newPlayer){
        (newPlayer ? axios.get('http://localhost/api/players/rivals/' + encodeURIComponent(player.gamerTag)) : Promise.resolve({data:{}}))
        .then(res => {            
            const rivals = res.data;
            if(newPlayer) this.setState({rivals: rivals});
            axios.all([
                axios.get('http://localhost/api/players/matchHistory/' + encodeURIComponent(player.gamerTag) +'/' + encodeURIComponent(rival ? rival : rivals[0].gamerTag)),            
                axios.get('http://localhost/api/players/player/' + encodeURIComponent(rival ? rival : rivals[0].gamerTag)),
                axios.get('http://localhost/api/players/records/' + encodeURIComponent(player.gamerTag) +'/' + encodeURIComponent(rival ? rival : rivals[0].gamerTag)),                
                axios.get('http://localhost/api/players/highestSet/' + encodeURIComponent(player.gamerTag) +'/' + encodeURIComponent(rival ? rival : rivals[0].gamerTag)),
                axios.get('http://localhost/api/players/lastMet/' + encodeURIComponent(player.gamerTag) +'/' + encodeURIComponent(rival ? rival : rivals[0].gamerTag))

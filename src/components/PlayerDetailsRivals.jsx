...
class PlayerDetailsRivals extends Component{
    ...
    getRivalData(player, rival, newPlayer){
        (newPlayer ? axios.get('http://localhost:' +  process.env.PORT + '/api/players/rivals/' + encodeURIComponent(player.gamerTag)) : Promise.resolve({data:{}}))
        .then(res => {            
            const rivals = res.data;
            if(newPlayer) this.setState({rivals: rivals});
            axios.all([
                axios.get('http://localhost:' +  process.env.PORT + '/api/players/matchHistory/' + encodeURIComponent(player.gamerTag) +'/' + encodeURIComponent(rival ? rival : rivals[0].gamerTag)),            
                axios.get('http://localhost:' +  process.env.PORT + '/api/players/player/' + encodeURIComponent(rival ? rival : rivals[0].gamerTag)),
                axios.get('http://localhost:' +  process.env.PORT + '/api/players/records/' + encodeURIComponent(player.gamerTag) +'/' + encodeURIComponent(rival ? rival : rivals[0].gamerTag)),                
                axios.get('http://localhost:' +  process.env.PORT + '/api/players/highestSet/' + encodeURIComponent(player.gamerTag) +'/' + encodeURIComponent(rival ? rival : rivals[0].gamerTag)),
                axios.get('http://localhost:' +  process.env.PORT + '/api/players/lastMet/' + encodeURIComponent(player.gamerTag) +'/' + encodeURIComponent(rival ? rival : rivals[0].gamerTag))
